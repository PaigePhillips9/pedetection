...
# https://github.com/i-pan/kaggle-rsna18/commit/8acd75952c0d6d2edeeadb8f0708d7808c2237a5
...
def inception_resnet_block(x, scale, block_type, block_idx, activation="relu"):
    ...
    mixed = keras.layers.Concatenate(axis=channel_axis, name=block_name + "_mixed")(branches)
    up = conv2d_bn(
        mixed,
        keras.backend.int_shape(x)[channel_axis],
        1,
        activation=None,
        use_bias=True,
...
def InceptionResNetV2Gray(
    ...
    channel_axis = 1 if keras.backend.image_data_format() == "channels_first" else 3
    x = keras.layers.Concatenate(axis=channel_axis, name="mixed_5b")(branches)
    ...
    x = keras.layers.Concatenate(axis=channel_axis, name="mixed_6a")(branches)
    ...
    branch_pool = keras.layers.MaxPooling2D(3, strides=2, padding="valid")(x)
    branches = [branch_0, branch_1, branch_2, branch_pool]
    x = keras.layers.Concatenate(axis=channel_axis, name="mixed_7a")(branches)
    ...
    # Create model
    model = keras.models.Model(inputs, x, name="inception_resnet_v2")
